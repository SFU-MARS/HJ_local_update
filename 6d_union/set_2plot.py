...
def plot_overlay_3dsets(g, data1, data2, plot_option):
    dims_plot = plot_option.dims_plot
    grid1, my_V1 = pre_plot(plot_option, g, data1)
    grid2, my_V2 = pre_plot(plot_option, g, data2)
    
    if len(dims_plot) == 3:
        complex_x = complex(0, grid1.pts_each_dim[0])
        complex_y = complex(0, grid1.pts_each_dim[1])
        complex_z = complex(0, grid1.pts_each_dim[2])
        mg_X, mg_Y, mg_Z = np.mgrid[grid1.min[0]:grid1.max[0]: complex_x, 
                                    grid1.min[1]:grid1.max[1]: complex_y, 
                                    grid1.min[2]:grid1.max[2]: complex_z]
        print("Ploting ... ")
